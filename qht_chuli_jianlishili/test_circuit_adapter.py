...
def test_adapter():
    ...
    try:
        ...
        for device_name, device_data in devices_dict.items():
            ...
            print(f"       引脚: {[p['name'] + '->' + (p['net'] or 'None') for p in device_data['pins']]}")
        
        print(f"\n   转换后的网络:")
        for net_name, net_data in nets_dict.items():
            print(f"     {net_name}: {[p['device'] + '.' + p['pin'] for p in net_data['pins']]}")
    ...
    try:
        ...
        constraint = detector.detect(devices_dict, nets_dict)
        
        print(f"   ✅ 对称检测成功!")
        print(f"   检测到 {len(constraint.symmetry_pairs)} 个对称对:")
